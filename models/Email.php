...
class Email extends Model
{
    ...
    private static function explodeSubject($subj) {
        $subj = trim($subj);
        return preg_match(PATTERN_SUBJ, $subj);
    }
    ...
    {
        $ml = imap_open($host->mail_host,$host->host_login,$host->host_password) or die('Cannot connect to Gmail: ' . imap_last_error());
    ...
    public function findRightMail($mail_data)
    {
        ...
        foreach ($mail_data as $arr) {
            ...
            if (($from == 'dima@udt.dp.ua') ||
                ...
                ($from == 'filippov.maksim@omega-auto.biz') ||
     ...
     * и возвращаем uid-письма, email, и дату ml
     */
    public function loadAttach($host, $uid, $path, $type = 'ml')
    {

        $arr_attach = Array();

        $inbox = imap_open($host->mail_host,$host->host_login,$host->host_password) or die('Cannot connect to Gmail: ' . imap_last_error());
        ...
        foreach ($part_array as $key => $attach) {
            if (($attach['part_object']->type == 3) &&
                ($attach['part_object']->disposition == 'attachment') &&
                ($attach['part_object']->ifdparameters == 1) &&
                ($attach['part_object']->dparameters[0]->attribute == 'filename')
            ) {
...
*/
    ...
    public function delMail($host, $uid) {
        ...
        //imap_mail_move ($inbox,$uid,'Trash',CP_UID);
